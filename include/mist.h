...
inline extern size_t MistCalculateIndex(size_t mist_bag_index, uint8_t* pointer);
...
inline size_t MistCalculateIndex(size_t mist_bag_index, uint8_t* pointer) {
    ...
    size_t xdiff = ((size_t)frame_base - (size_t)MQBI.base_ptr);
#ifdef MIST_VERBOSE_DEBUG
    printf("Returning %llu from MistCalculateIndex(%llu, %p), frame_base = 0x%lx, xdiff = %llu\n", (xdiff / MIST_FRAME_SIZE), mist_bag_index, pointer, frame_base, xdiff);
#endif
...
inline void MistZeroFrame(size_t mist_bag_index, size_t mist_frame_index) {
    ...
    if (!MQBI.init) {
#ifdef MIST_VERBOSE_DEBUG
        printf("Returning from MistZeroFrame(%llu, %llu) because the bag at this index is not initialized\n", mist_bag_index, mist_frame_index);
#endif
        return;
    }

    uint8_t* framebase = MQBI.base_ptr + (mist_frame_index * MIST_FRAME_SIZE);

#ifdef MIST_VERBOSE_DEBUG
    printf("Attempting to zero from framebase 0x%lx, frame_index = %llu\n", framebase, mist_frame_index);
#endif
...
#define MIST_BAD_ALLOC ((void*)-1)
    ...
    if (result == MIST_BAD_ALLOC) {
        printf("MIST_BAD_ALLOC returned when allocating 0x%lx in MistMemMapFrame() - Windows\n", (uint64_t)frame_base_ptr);
...
#endif
    if (result == MIST_BAD_ALLOC) {
        printf("MIST_BAD_ALLOC returned when allocating 0x%lx in MistMemMapFrame() - POSIX\n", (uint64_t)frame_base_ptr);
        return MIST_NULLPTR_RETURNED;
    }
    else {
#ifdef MIST_VERBOSE_DEBUG
...
#else
    munmap((void*)(frame_base_ptr), MIST_FRAME_SIZE);
#endif
    return MIST_OK;
}
...
int MistInit() {
    ...
    if (result != MIST_OK) {
        printf("Call to allocate genesis frame on default bag returned NULL in MistInit()\n");
        return result;
    }
    ...
    if (result2 != MIST_OK) {
        printf("Call to commit genesis frame on default bag returned NULL in MistInit()\n");
        return result2;
    }
#elif !defined(MIST_OS_WINDOWS)
    int result = MistMemMapFrame(next_major_alloc_ptr, MIST_MAX_BAG_ALLOC, false, false, false);
    // Make sure we actually received VAS
    if (result != MIST_OK) {
        printf("Call to allocate genesis frame on default bag returned %d in MistInit(): %d\n", result, (int)errno);
        return result;
    }

    int result2 = MistMemMapFrame(next_major_alloc_ptr, MIST_FRAME_SIZE, true, false, false);
    // Make sure we actually received VAS
    if (result2 != MIST_OK) {
        printf("Call to commit genesis frame on default bag returned %d in MistInit(): %d\n", result2, (int)errno);
        return result2;
    }
#endif

    // Bookeeping
    MQCBI.base_ptr = next_major_alloc_ptr;
    MQCBI.frames = 1;
...
int MistAllocateBag(size_t max_frames, uint16_t type_width, uint16_t type_align) {
    ...
    MQCBI.alloc.current_ptr = MQCBI.base_ptr;
...
#ifdef MIST_VERBOSE_DEBUG
    printf("In MistNew(%lu) allocation_size = %lu, next_bump_loc = 0x%lx, next_bump_index = %lu, \
        current_bump_index = %lu\n", mist_bag_index, allocation_size, \
        next_bump_loc, next_bump_index, current_bump_index);
#endif
    uint8_t* retptr = NULL;
