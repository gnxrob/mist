...
int main(int argc, char* argv[]) {
    ...
    printf("Bag #1 index is 0x%x (MAX_FRAMES, 8 Byte Size, 8 Byte Align) @ 0x%llx\n", (uint32_t)bag1_idx, (uint64_t)mist_manager.bag[bag1_idx].bag_base_ptr);
    printf("Bag #2 index is 0x%x (MAX_FRAMES, 4 Byte Size, 4 Byte Align) @ 0x%llx\n", (uint32_t)bag2_idx, (uint64_t)mist_manager.bag[bag2_idx].bag_base_ptr);
    printf("Bag #3 index is 0x%x (MAX_FRAMES, 12 Byte Size, 16 Byte Align) @ 0x%llx\n", (uint32_t)bag3_idx, (uint64_t)mist_manager.bag[bag3_idx].bag_base_ptr);

    for (z = 0; z < 0x4000; z++) {
        pointers64[z] = (uint64_t*)MistNew(bag1_idx, 0);
        if (pointers64[z] == NULL) {
            printf("Null pointer returned on pointers64! 0x%llx at index %lld\n", pointers64[z], z);
            break;
        }
        *pointers64[z] = (uint64_t)z;
        ...
        if (pointers32[z] == NULL) {
            printf("Null pointer returned on pointers32! 0x%llx at index %lld\n", pointers32[z], z);
            break;
        }
        *pointers32[z] = (uint32_t)z;
        ...
        if (pointers12byte[z] == NULL) {
            printf("Null pointer returned! 0x%llx at index %lld\n", pointers12byte[z], z);
            break;
        }
    ...
    printf("Clearing bag %lld, frame %lld\n", bag2_idx, 0);

    MistZeroFrame(bag2_idx, 0);

    for (z = 0; z < 10; z++) {

        printf("pointers64 loc[%lld] == (0x%p) 0x%x pointers32 loc[%lld] == (0x%p) 0x%x pointers12byte loc[%lld] == (0x%p) .b8 == 0x%x, .b4 == 0x%x\n", 
